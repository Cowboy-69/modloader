...
static auto xinit = initializer([](DataPlugin* plugin_ptr)
{
    if (plugin_ptr->loader->game_id == MODLOADER_GAME_RE3)
    {
        ...
        plugin_ptr->particlecfg_detour.SetFileDetour(LoadFileDetourRE());
    }
    else if (plugin_ptr->loader->game_id == MODLOADER_GAME_REVC)
    {
        ...
        plugin_ptr->modloader_reVC->callback_table->LoadFile_ParticleCfg = plugin_ptr->REVCDetour_LoadFile_ParticleCfg;
        plugin_ptr->particlecfg_detour.SetFileDetour(LoadFileDetourRE());
    }
    else
    {
